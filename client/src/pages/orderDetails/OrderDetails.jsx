...
const OrderDetails = () => {
  ...
  const orderCancel=async()=>{
    try {
      const res=await AunthenticatedAPI.patch('/orders/cancel',{id})
      if(res.status===200){
        navigate('/myorders')
      }
    } catch (error) {
  ...
  const orderDelete=async()=>{
    try {
      const res=await AunthenticatedAPI.delete(`/orders/${id}`)
      if(res.status===200){
        navigate('/myorders')
      }
    } catch (error) {
      ...
      <div className=" flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
        <div className="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
          ...
          <div className="flex w-full justify-center items-center md:justify-start md:items-start gap-1">
          {
              ...
              <>
                  <button className="mt-6 md:mt-0 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent dark:text-white py-5 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base leading-4 text-gray-800">Edit Order</button>
                 <button onClick={orderCancel} className="mt-6 md:mt-0 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent dark:text-white py-5 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base leading-4 text-gray-800">Cancel Order</button>
              </>
            }
          </div>
          <div className="flex w-full justify-center items-center md:justify-start md:items-start gap-1">
            {
              filteredSingleOrderDetail[0]?.orderStatus !='cancelled' &&
            <button onClick={orderDelete} className="mt-6 md:mt-0 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent dark:text-white py-5 hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base leading-4 bg-[red] text-white">Delete Order</button>
            }
          </div>
        </div>
      </div>
